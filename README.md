# Projet de Rénovation - Salle de Bain.

**Date de début:** 29 décembre 2024
**Statut:** En planification

---

## 💰 BUDGET ESTIMÉ

| Catégorie                | Montant        |
| ------------------------ | -------------- |
| Carrelage et Revêtements | 4 135$         |
| Sanitaires               | 2 500$         |
| Meubles                  | 1 500$         |
| Électricité              | 180$           |
| Accessoires              | 100$           |
| Peinture                 | 100$           |
| **SOUS-TOTAL MATÉRIAUX** | **8 515$**     |
|                          |                |
| Imprévus (10%)           | 852$           |
|                          |                |
| **TOTAL ESTIMÉ**         | **9 367$**     |

---

## Informations Générales

### Objectifs du Projet
- [ ] Définir les objectifs principaux de la rénovation
- [ ] Établir le budget global
- [ ] Déterminer le calendrier prévisionnel

### Contraintes
- Budget maximum : _À définir_
- Délai souhaité : _À définir_
- Contraintes techniques : _À définir_

---

## Budget Prévisionnel

### Récapitulatif

| Catégorie | Budget Prévu | Dépensé | Restant |
|-----------|--------------|---------|---------|
| Matériaux | - | - | - |
| Main d'œuvre | - | - | - |
| Équipements | - | - | - |
| Imprévus (10-15%) | - | - | - |
| **TOTAL** | - | - | - |

---

## Matériaux et Équipements

### Sanitaires

| Article | Quantité | Prix Unitaire | Prix Total | Fournisseur | Acheté |
|---------|----------|---------------|------------|-------------|--------|
| WC suspendu Eridanus Hampton 1.6 GPF avec siège | 1 | 600,00$ | 600,00$ | Wayfair | ☐ |
| Base douche 32×48 (pierre/effet pierre) | 1 | 300,00$ | 300,00$ | - | ☐ |
| Cabine douche (parois + porte verre) | 1 | 300,00$ | 300,00$ | - | ☐ |
| Baignoire autoportante 71 po avec plateau bambou | 1 | 700,00$ | 700,00$ | Costco | ☐ |
| Robinetterie (lavabo + baignoire + douche) | 1 | 300,00$ | 300,00$ | - | ☐ |
| **SOUS-TOTAL SANITAIRES** | - | - | **2 500,00$** | - | - |

### Carrelage et Revêtements

| Article                  | Surface (pi²)    | Prix/pi² | Prix Total | Fournisseur | Acheté |
| ------------------------ | ---------------- | -------- | ---------- | ----------- | ------ |
| Carrelage sol            | 97 (88 + 10%)    | 10,00$   | 970$       | - | ☐      |
| Carrelage mur            | 304 (276 + 10%)  | 10,00$   | 3 040$     | - | ☐      |
| Colle/Mortier            | 5 sacs (401 pi²) | 25,00$   | 125$       | - | ☐      |
| Joints sol               | -                | -        | -          | - | ☐      |
| Joints mur               | -                | -        | -          | - | ☐      |
| **SOUS-TOTAL CARRELAGE** | -                | -        | **4 135$** | - | -      |

### Plomberie

| Article | Quantité | Prix Unitaire | Prix Total | Fournisseur | Acheté |
|---------|----------|---------------|------------|-------------|--------|
| Tuyauterie | - | - | - | - | ☐ |
| Siphons | - | - | - | - | ☐ |
| Raccords | - | - | - | - | ☐ |

### Électricité

| Article | Quantité | Prix Unitaire | Prix Total | Fournisseur | Acheté |
|---------|----------|---------------|------------|-------------|--------|
| Ventilateur salle de bain | 1 | 80,00$ | 80,00$ | | ☐ |
| Luminaires | - | - | 100,00$ | - | ☐ |
| Prises électriques | - | - | - | - | ☐ |
| Interrupteurs | - | - | - | - | ☐ |
| Câblage | - | - | - | - | ☐ |
| **SOUS-TOTAL ÉLECTRICITÉ** | - | - | **180,00$** | - | - |

### Meubles et Rangements

| Article                        | Quantité | Prix Unitaire | Prix Total    | Fournisseur | Acheté |
| ------------------------------ | -------- | ------------- | ------------- | ----------- | ------ |
| Vanité double lavabo           | 1        | 1 000,00$     | 1 000,00$     |             | ☐      |
| Armoire pharmacie avec miroirs | 1        | 500,00$       | 500,00$       |             | ☐      |
| Rangements muraux              | -        | -             | -             | - | ☐      |
| **SOUS-TOTAL MEUBLES**         | -        | -             | **1 500,00$** | - | -      |

### Accessoires et Finitions

| Article | Quantité | Prix Unitaire | Prix Total | Fournisseur | Acheté |
|---------|----------|---------------|------------|-------------|--------|
| Peinture | - | - | - | - | ☐ |
| Porte-serviettes | - | - | - | - | ☐ |
| Distributeur savon | - | - | - | - | ☐ |
| Autres accessoires | - | - | - | - | ☐ |

---

## Planning des Travaux

### Phase 1 : Travaux de Structure
- [ ] Modification de la plomberie 
- [ ] Modification de l'électricité 
- [ ] Traitement de l'humidité
- [ ] Préparation des murs et sols

### Phase 2 : Installation
- [ ] Pose du carrelage sol
- [ ] Pose du carrelage mur
- [ ] Installation de la douche/baignoire
- [ ] Installation du lavabo
- [ ] Installation du WC
- [ ] Installation de la robinetterie
### Phase 3 : Finitions
- [ ] Joints de carrelage
- [ ] Peinture
- [ ] Installation des luminaires
- [ ] Pose des meubles
- [ ] Installation des accessoires

_



---

## Mesures et Plans

### Dimensions Générales
- Longueur totale : 11 pi
- Largeur totale : 8 pi
- Hauteur sous plafond : 8 pi (96 po)
- **Surface au sol : 88 pi²**
- Douche : 32 po × 48 po (10,67 pi² inclus dans sol)

### Mesures des Murs

#### Mur 1 - Vanité et Toilette
- Longueur : 11 pi
- Hauteur : 94 po (7 pi 10 po)
- **Surface : 86 pi²**

#### Mur 2 - Baignoire Auto-portante et Douche
- Longueur totale : 11 pi
- Hauteur carrelage : 96 po (8 pi - sol au plafond)
- Baignoire autoportante (carrelage complet derrière)
- Douche vitrée 32×48 (mur fond carrelé)
- **Surface totale Mur 2 : 88 pi²**

#### Mur 3 - Latéral (avec fenêtre)
- Longueur : 8 pi
- Hauteur : 94 po (7 pi 10 po)
- Fenêtre : 2 pi × 2 pi (4 pi²)
- Surface totale : 63 pi²
- **Surface à carreler : 59 pi²** (sans la fenêtre)

#### Mur 4 - Entrée (avec porte)
- Longueur : 8 pi (96 po)
- Hauteur : 94 po (7 pi 10 po)
- Porte : 30 po × 94 po (20 pi²)
- Surface totale : 63 pi²
- **Surface à carreler : 43 pi²** (sans la porte)

### Points Techniques
- Arrivée d'eau : _À noter_
- Évacuation : _À noter_
- Ventilation : _À noter_
- Alimentation électrique : _À noter_

### Schémas et Photos
_Insérer ici les photos de l'état actuel et les plans souhaités_

---


### Liens Produits
- **WC suspendu Eridanus Hampton** (Wayfair): https://www.wayfair.ca/Eridanus--Eridanus-Hampton-1.6-Gallons-GPF-Elongated-Wall-Mounted-Wall-Hung-Toilets-Seat-Included--ERIWT4210-L587-K~ANIU1675.html

![WC suspendu Eridanus Hampton](https://assets.wfcdn.com/im/09750803/resize-h1200-w1200%5Ecompr-r85/2885/288526018/Toilettes+allong%C3%A9es+murales+%C3%A0+double+chasse+0%2C8+%2F+1%2C6+GPF+avec+r%C3%A9servoir+encastr%C3%A9+Hampton%2C+convient+%C3%A0+2+ou+6+clous-101757845.jpg)

- **Baignoire autoportante 71 po** (Costco): https://www.costco.ca/streamline---baignoire-autoportante-71-po-avec-un-plateau-de-baignoire-extensible-en-bambou.product.4000266777.html?preselect=couleur%3Ablanc

![Baignoire autoportante 71 po Costco](https://bfasset.costco-static.com/U447IH35/as/98jm3f6b76q9977hpcbmc2c/4000266777-894_white_3?auto=webp&format=jpg&width=1200&height=1200&fit=bounds&canvas=1200,1200)



---

## 📋 RÉSUMÉ DES ACHATS - LISTE DE MAGASINAGE

### Carrelage et Revêtements

| Article | Quantité | Prix Unitaire | Prix Total | À acheter |
|---------|----------|---------------|------------|-----------|
| Carrelage sol (petits carreaux) | 97 pi² | 10,00$/pi² | 970$ | ☐ |
| Carrelage mur (TRÈS GRANDS carreaux) | 304 pi² | 10,00$/pi² | 3 040$ | ☐ |
| Colle/Mortier | 5 sacs | 25,00$ | 125$ | ☐ |
| Coulis/Joints (à estimer) | - | - | - | ☐ |
| **SOUS-TOTAL CARRELAGE** | - | - | **4 135$** | - |

### Sanitaires et Plomberie

| Article | Quantité | Prix Unitaire | Prix Total | À acheter |
|---------|----------|---------------|------------|-----------|
| WC suspendu blanc (Yuan Home) | 1 | 600,00$ | 600,00$ | ☐ |
| Base douche 32×48 (pierre/effet pierre) | 1 | 300,00$ | 300,00$ | ☐ |
| Cabine douche (parois + porte verre) | 1 | 300,00$ | 300,00$ | ☐ |
| Baignoire autoportante blanche | 1 | 700,00$ | 700,00$ | ☐ |
| Robinetterie lavabo (2 robinets) | 2 | - | _À déterminer_ | ☐ |
| Robinetterie douche | 1 | - | _À déterminer_ | ☐ |
| Robinetterie baignoire | 1 | - | _À déterminer_ | ☐ |
| **SOUS-TOTAL SANITAIRES** | - | - | **2 500,00$** | - |

### Meubles et Rangements

| Article | Quantité | Prix Unitaire | Prix Total | À acheter |
|---------|----------|---------------|------------|-----------|
| Vanité double lavabo | 1 | 1 000,00$ | 1 000,00$ | ☐ |
| Armoire pharmacie avec miroirs | 1 | 500,00$ | 500,00$ | ☐ |
| **SOUS-TOTAL MEUBLES** | - | - | **1 500,00$** | - |

### Électricité et Ventilation

| Article | Quantité | Prix Unitaire | Prix Total | À acheter |
|---------|----------|---------------|------------|-----------|
| Ventilateur salle de bain | 1 | 80,00$ | 80,00$ | ☐ |
| Luminaires/Spots encastrés | - | - | 100,00$ | ☐ |
| Prises et interrupteurs (si nécessaire) | - | - | - | ☐ |
| **SOUS-TOTAL ÉLECTRICITÉ** | - | - | **180,00$** | - |

### Fenêtre

| Article | Quantité | Prix | À acheter |
|---------|----------|------|-----------|
| Fenêtre 2×2 pi avec cadre blanc | 1 | _Existante_ | - |

### Divers

| Article | Quantité | Prix | À acheter |
|---------|----------|------|-----------|
| Peinture (zones non carrelées) | - | - | ☐ |
| Accessoires (porte-serviettes, etc.) | - | - | ☐ |

---

## 💰 BUDGET TOTAL ACTUEL

| Catégorie                | Montant        |
| ------------------------ | -------------- |
| Carrelage et Revêtements | 4 135$         |
| Sanitaires               | 2 500$         |
| Meubles                  | 1 500$         |
| Électricité              | 180$           |
| Accessoires              | 100$           |
| Peinture                 | 100$           |
| **SOUS-TOTAL MATÉRIAUX** | **8 515$**     |
|                          |                |
| Imprévus (10%)           | 852$           |
|                          |                |
| **TOTAL ESTIMÉ**         | **9 367$**     |

---

## 📝 NOTES IMPORTANTES POUR LES ACHATS

### Spécifications Carrelage
- **Quantité totale** : 401 pi² (avec 10% de perte inclus)

### Spécifications Sanitaires
- **WC** : Suspendu, blanc, avec siège fermeture douce
- **Baignoire** : Autoportante (freestanding), blanche
- **Douche** : Base 32×48 po en pierre, cabine vitrée
- **Robinetterie** : Chrome moderne

### Spécifications Meubles
- **Vanité** : Double lavabo, contemporaine
- **Armoire** : Pharmacie avec miroirs

### Spécifications Électricité
- **Ventilateur** : Extracteur salle de bain
- **Luminaires** : Spots encastrés au plafond et appliques 3murales.

---


